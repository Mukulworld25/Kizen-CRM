-- Kizen CRM — Migration 012: Structured Data Auto-Intake System

-- 1. Schema Additions
ALTER TABLE IF EXISTS institute_expenses ADD COLUMN IF NOT EXISTS vendor TEXT;

-- 2. Data Intake Tables
CREATE TABLE IF NOT EXISTS data_templates (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  section TEXT NOT NULL UNIQUE CHECK (section IN ('leads', 'finance', 'institutions')),
  required_columns JSONB NOT NULL,
  optional_columns JSONB DEFAULT '[]'::jsonb,
  dedup_keys JSONB NOT NULL,
...
CREATE TABLE IF NOT EXISTS import_audit_log (
  ...
  filename_source TEXT NOT NULL,
  row_count_attempted INT NOT NULL DEFAULT 0,
  row_count_imported INT NOT NULL DEFAULT 0,
  row_count_rejected_skipped INT NOT NULL DEFAULT 0,
  uploaded_by UUID REFERENCES users(id) ON DELETE SET NULL,
  template_matched BOOLEAN DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS sheet_connections (
  ...
  template_section TEXT NOT NULL REFERENCES data_templates(section) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS data_intake_settings (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  source TEXT UNIQUE NOT NULL CHECK (source IN ('manual_upload', 'sheets_sync', 'meta_ads', 'google_ads')),
...
-- Seed default locked templates
INSERT INTO data_templates (section, required_columns, optional_columns, dedup_keys) VALUES
  (
    'leads',
    '["full_name", "mobile"]'::jsonb,
    '[{"name": "email", "default": null}, {"name": "parent_name", "default": null}, {"name": "parent_contact", "default": null}, {"name": "city", "default": null}, {"name": "school_college", "default": null}, {"name": "class_year", "default": null}, {"name": "graduation_year", "default": null}, {"name": "graduation_degree", "default": null}, {"name": "source", "default": "website"}, {"name": "status", "default": "new_lead"}, {"name": "priority", "default": "medium"}, {"name": "notes", "default": null}]'::jsonb,
    '["mobile", "email"]'::jsonb
  ),
  (
    'finance',
    '["category", "amount", "expense_date", "vendor"]'::jsonb,
    '[{"name": "notes", "default": null}]'::jsonb,
    '["expense_date", "amount", "category", "vendor"]'::jsonb
  ),
  (
    'institutions',
    '["name", "type"]'::jsonb,
    '[{"name": "address", "default": null}, {"name": "city", "default": null}, {"name": "contact_person", "default": null}, {"name": "contact_phone", "default": null}, {"name": "contact_email", "default": null}, {"name": "mou_status", "default": "not_started"}]'::jsonb,
    '["name", "type"]'::jsonb
  )
...
ALTER TABLE ad_sync_connections ENABLE ROW LEVEL SECURITY;
ALTER TABLE data_intake_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY data_templates_select ON data_templates FOR SELECT USING (TRUE);
CREATE POLICY import_audit_log_select ON import_audit_log FOR SELECT USING (TRUE);
CREATE POLICY sheet_connections_all ON sheet_connections FOR ALL USING (TRUE);
CREATE POLICY ad_sync_connections_all ON ad_sync_connections FOR ALL USING (TRUE);
CREATE POLICY data_intake_settings_all ON data_intake_settings FOR ALL USING (TRUE);

-- 3. Core RPC Function: process_intake_batch
...
BEGIN
  ...
  IF v_source_enabled IS NOT TRUE THEN
    INSERT INTO import_audit_log (
      ...
      row_count_rejected_skipped, uploaded_by, template_matched, status, error_reason
    ) VALUES (
      p_section, p_filename, jsonb_array_length(p_rows), 0, jsonb_array_length(p_rows),
      p_uploaded_by, FALSE, 'rejected', 'This intake source is currently turned off'
    );
  ...
  IF v_req_cols IS NULL THEN
    INSERT INTO import_audit_log (
      ...
      p_section, p_filename, jsonb_array_length(p_rows), 0, jsonb_array_length(p_rows),
      p_uploaded_by, FALSE, 'rejected', 'No template found for section: ' || p_section
    );
  ...
  IF array_length(v_missing_req, 1) > 0 OR array_length(v_unknown_cols, 1) > 0 THEN
    ...
    BEGIN
      ...
      INSERT INTO import_audit_log (
        ...
        p_section, p_filename, jsonb_array_length(p_rows), 0, jsonb_array_length(p_rows),
        p_uploaded_by, FALSE, 'rejected', v_err
      );
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    ELSIF p_section = 'finance' THEN
      ...
      INSERT INTO institute_expenses (
        category, amount, expense_date, vendor, notes, created_by
      ) VALUES (
        trim(v_row->>'category'),
    ...
    ELSIF p_section = 'institutions' THEN
      ...
      INSERT INTO institutions (
        ...
        contact_email, mou_status, assigned_bdm_id
      ) VALUES (
        trim(v_row->>'name'),
        trim(v_row->>'type'),
  ...
  INSERT INTO import_audit_log (
    section, filename_source, row_count_attempted, row_count_imported,
    row_count_rejected_skipped, uploaded_by, template_matched, status
  ) VALUES (
    p_section, p_filename, v_attempted_count, v_imported_count,
    v_skipped_count, p_uploaded_by, TRUE, 'success'
  );
...
EXCEPTION WHEN OTHERS THEN
  INSERT INTO import_audit_log (
    ...
    p_uploaded_by, TRUE, 'rejected', SQLERRM
  );
