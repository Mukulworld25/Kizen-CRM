...
async function main() {
  ...
  const { data: lead, error: le } = await O2.from('leads').insert({
    ...
    assigned_counselor_id: counselor.id, status: 'new_lead', priority: 'high', notes: 'E2E test'
  }).select().single()
  if (le) { l(`  Lead create: ${le.message}`); finish(); return }
  l(`  Created: ${lead.full_name} -> ${lead.status}`)

  for (const s of ['contacted','follow_up','demo_booked','demo_attended','negotiation','registration_pending','fee_pending','converted']) {
    const { error: ue } = await O2.from('leads').update({ status: s }).eq('id', lead.id)
    if (ue) { l(`    -> ${s}: ${ue.message}`); finish(); return }
    l(`    -> ${s}`)
  }
  ...
  let pass = true
  if (vLead.data?.status === 'converted') l(`  Lead pipeline: new_lead→contacted→follow_up→demo_booked→demo_attended→negotiation→registration_pending→fee_pending→converted ✓`)
