...
CREATE POLICY lead_activities_select ON lead_activities FOR SELECT USING (
  EXISTS (
    ...
    AND (
      get_user_role() IN ('owner','admin','reception')
      OR l.assigned_counselor_id = get_user_id()
    )
  )
);
...
CREATE POLICY follow_ups_select ON follow_ups FOR SELECT USING (
  ...
  OR EXISTS (
    SELECT 1 FROM leads l WHERE l.id = follow_ups.lead_id
    AND l.assigned_counselor_id = get_user_id()
  )
);
...
CREATE POLICY documents_select ON documents FOR SELECT USING (
  (entity_type = 'lead' AND EXISTS (
    SELECT 1 FROM leads l WHERE l.id = documents.entity_id
    AND (get_user_role() IN ('owner','admin','reception') OR l.assigned_counselor_id = get_user_id())
  ))
  OR (entity_type = 'student' AND EXISTS (
    SELECT 1 FROM students s WHERE s.id = documents.entity_id
    AND (
      get_user_role() IN ('owner','admin','counselor','accounts')
      OR s.faculty_id = get_user_id()
      OR EXISTS (SELECT 1 FROM leads l WHERE l.id = s.lead_id AND l.assigned_counselor_id = get_user_id())
    )
  ))
);
...
CREATE POLICY students_select ON students FOR SELECT USING (
  ...
  OR faculty_id = get_user_id()
  OR EXISTS (
    SELECT 1 FROM leads l WHERE l.id = students.lead_id
    AND l.assigned_counselor_id = get_user_id()
  )
);
...
CREATE POLICY attendance_select ON attendance FOR SELECT USING (
  get_user_role() IN ('owner','admin','counselor','accounts')
  OR EXISTS (
    SELECT 1 FROM batches b WHERE b.id = attendance.batch_id AND b.faculty_id = get_user_id()
  )
  OR EXISTS (
    SELECT 1 FROM students s WHERE s.id = attendance.student_id
    AND s.faculty_id = get_user_id()
  )
);
